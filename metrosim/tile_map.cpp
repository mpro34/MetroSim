...
/* Load map from disk */
void TileMap::load(const std::string& filename, unsigned int width, unsigned int height,
...
void TileMap::draw(sf::RenderWindow& window, float dt)
{
...
void TileMap::updateDirection(TileType tileType)
{
    for (int y = 0; y < this->height; ++y)
    {
        for (int x = 0; x < this->width; ++x)
...
void TileMap::dfs(std::vector<TileType>& whitelist, sf::Vector2i pos, int label, int regionType = 0)
{
    ...
    dfs(whitelist, pos + sf::Vector2i(-1, 0), label, regionType);
    dfs(whitelist, pos + sf::Vector2i(0, 1), label, regionType);
    dfs(whitelist, pos + sf::Vector2i(1, 0), label, regionType);
    dfs(whitelist, pos + sf::Vector2i(0, -1), label, regionType);
}

void TileMap::findConnectedRegions(std::vector<TileType> whitelist, int regionType = 0)
{
    int regions{ 1 };
    for (auto& tile : this->tiles)
    {
    ...
    for (int y = 0; y < this->height; ++y)
    {
        for (int x = 0; x < this->width; ++x)
        {
            ...
            for (auto type : whitelist)
            {
                ...
                {
                    dfs(whitelist, sf::Vector2i(x, y), regions++, regionType);
                }
            }
        }
    }
